...
# The IP of the node we are on (should be the IP of k8s-worker-1)
...
# The IP address of the other node (k8s-worker-2) that we will use to test connectivity.
...
# We will now set up the routing on the host machine to forward traffic meant for
# the namespaces on the other node (k8s-worker-2) to the other node itself.
echo "Setting up routing on host machine to forward traffic to other node..."
sudo ip route add $TO_BRIDGE_SUBNET via $TO_NODE_IP dev enp0s8

# In order for cross-node communication to work, we also need to set up routing on the
# other node (k8s-worker-2). The cross-node tests won't work until we do that, so,
# go do that from the other file.

echo "Setup on this node (k8s-worker-1) is complete. Please run the setup on the other node (k8s-worker-2)."
echo "After setting up the other node, run the test scripts to verify connectivity."