...
# the namespaces on the other node (k8s-worker-1) to the other node itself.
...
# other node (k8s-worker-1). The cross-node tests won't work until we do that, so,
# go do that from the other file.

echo "Setup on this node (k8s-worker-2) is complete. Please run the setup on the other node (k8s-worker-1)."
